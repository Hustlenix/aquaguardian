...
const bubbleFragmentShader = `
  ...
  void main() {
    ...
    float r = length(p);
    // Soft translucent body + bright rim for a bubble look.
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      if (popTimers.current[i] <= 0) {
        // Rising with a gentle perpendicular wobble.
...
<<<<<<< HEAD
        ...
        if (pos[i * 3 + 1] > 8) {
          pos[i * 3 + 1] = -3
=======
        pos[i * 3] += Math.sin(t * 0.6 + phases.current[i]) * delta * 0.18
        pos[i * 3 + 2] += Math.cos(t * 0.5 + phases.current[i] * 1.3) * delta * 0.1
        sizes[i] = baseSizes.current[i] * (1 + Math.sin(t * 0.4 + i) * 0.15)
        alphas[i] = 1

        // Reached the surface -> pop.
        if (pos[i * 3 + 1] > SURFACE_Y) {
          popTimers.current[i] = POP_DURATION
          pos[i * 3 + 1] = SURFACE_Y
        }
      } else {
        // Pop: shrink down + fade out.
